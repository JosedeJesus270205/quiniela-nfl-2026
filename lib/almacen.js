...
/**
 * Deja un pick puesto y firmado de un jalon, por correccion del
 * administrador. Guarda quien y cuando, para que quede rastro de que ese pick
 * no lo puso el jugador.
 */
function corregirPick(usuarioId, partidoId, eleccion) {
  var mios = picksDe(usuarioId);
  mios[partidoId] = {
    eleccion: eleccion,
    confirmado: true,
    actualizado: reloj.sello(),
    corregidoPorAdmin: reloj.sello()
  };
  guardar();
  return mios[partidoId];
}
