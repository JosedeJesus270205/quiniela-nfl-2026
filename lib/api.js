 ...
 */
/** Los records del contexto, calculados la primera vez que se piden. */
function recordsDe(contexto) {
  if (!contexto.records) contexto.records = reglas.records(contexto.calendario, contexto.resultados);
  return contexto.records;
}
...
function verPartido(partido, contexto) {
  ...
  return {
    ...
    record: {
      local: reglas.textoRecord(recordsDe(contexto)[partido.local.abbr]),
      visitante: reglas.textoRecord(recordsDe(contexto)[partido.visitante.abbr])
    },
    puntos: valor,
    ...
    resultado: resultado && resultado.final ? {
      ...
      // Con prorroga, el marcador que conto fue el del minuto 60.
      prorroga: !!resultado.prorroga,
      // Solo se avisa el minuto 60 cuando fue el que conto.
      normal: resultado.prorroga && resultado.tiempoNormal &&
              reglas.cuentaDe(resultado, partido.id) !== 'final'
        ? reglas.marcadorQueCuenta(resultado, partido.id) : null,
      ganador: reglas.ganadorDeResultado(resultado, partido.id)
    } : null,
...
function contextoDe(usuario, ahora, enVivo) {
  ...
  return {
    ...
    vivo: enVivo || null,
    // El record de cada equipo al dia de hoy. Se arma una vez por peticion.
    records: null,
