...
function contextoDe(usuario, ahora) {
...
function tabla(ctx) {
  ...
  reglas.semanas(calendario).forEach(function (n) {
    ...
    ganadores.forEach(function (g) { ganadas[g.id] = (ganadas[g.id] || 0) + 1; });
  ...
  general.forEach(function (f) {
    // Lo que lleva depositado cada quien NO va aqui. La tabla la ve toda la
    // quiniela, y cuanto dinero trae metido cada uno es asunto suyo y del
    // administrador. Cada jugador ve el suyo en Mi cuenta; el administrador
    // los ve todos en su panel. Tampoco se manda al navegador: si solo se
    // escondiera en pantalla, el dato seguiria viajando y se leeria en las
    // herramientas del navegador.
    f.semanasGanadas = ganadas[f.id] || 0;
  });
