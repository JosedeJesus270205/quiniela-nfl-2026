...
function resumenSemanas(ctx) {
  return reglas.semanas(ctx.calendario).map(function (n) {
    var cierre = reglas.cierreDeSemana(ctx.calendario, n, ctx.config);
    ...
    return {
      ...
      cerrada: cierre ? ctx.ahora.getTime() >= cierre.getTime() : true,
...
function semana(ctx) {
  ...
  var cierre = reglas.cierreDeSemana(c.calendario, n, c.config);

  return {
    semana: n,
    ahora: c.ahora.toISOString(),
    cierre: cierre ? cierre.toISOString() : null,
    cerrada: cierre ? c.ahora.getTime() >= cierre.getTime() : true,
...
function tabla(ctx) {
  ...
  general.forEach(function (f) {
    var pagos = almacen.pagosDe(f.id);
    f.pagado = reglas.totalPagado(pagos);
    f.alCorriente = f.pagado >= config.totalTemporada;
