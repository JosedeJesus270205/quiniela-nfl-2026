...
function semana(ctx) {
  ...
  var siguiente = reglas.proximoCierre(calendario, n, ahora, config);
  ...
  return {
    ...
    partidos: juegos.map(function (p) {
      ...
      return {
        ...
        cierre: reglas.cierreDePartido(p, config).toISOString(),
        ...
        resultado: r && r.final ? {
          ...
          ganador: reglas.ganadorDeResultado(r, p.id)
        } : null
      };
    })
  };
...
function picksDeSemana(ctx) {
  ...
  return {
    semana: n,
    partidos: juegos.map(function (p) {
      // Las siglas viajan aparte porque la rejilla enseña a QUE EQUIPO le
      // entro cada quien, no si eligio "local" o "visitante".
      return {
        id: p.id,
        etiqueta: p.visitante.abbr + ' @ ' + p.local.abbr,
        local: p.local.abbr,
        visitante: p.visitante.abbr,
        inicio: p.inicio
      };
    }),
...
/**
 * Corrige el pick de un jugador en UN partido, por equivocacion al capturar.
 *
 * LA REGLA DE ORO SIGUE MANDANDO: esto solo funciona mientras el partido esta
 * abierto. Si ya arranco, o si ya paso su cierre, se niega. Asi esta funcion
 * no sirve para colar un pick con informacion que el resto no tenia, que es
 * justo lo que la quiniela no puede permitir.
 *
 * Queda rastro en el pick (corregidoPorAdmin) de que no lo puso el jugador.
 */
function corregirPick(ctx) {
  api.exigirAdmin(ctx);
  var config = configuracion.cargar();
  var calendario = almacen.calendario();

  var usuario = almacen.buscarPorId(String(ctx.cuerpo.usuarioId || ''));
  if (!usuario) throw api.error(404, 'Ese jugador no existe.');

  var partido = calendario.partidos.find(function (p) {
    return p.id === String(ctx.cuerpo.partidoId || '');
  });
  if (!partido) throw api.error(404, 'Ese partido no existe.');

  var eleccion = String(ctx.cuerpo.eleccion || '');
  if (reglas.ELECCIONES.indexOf(eleccion) === -1) {
    throw api.error(400, 'La eleccion tiene que ser local, visitante o empate.');
  }

  var estado = reglas.estadoDePartido(calendario, partido, reloj.ahora(), config);
  if (estado !== 'abierto') {
    throw api.error(409, estado === 'iniciado'
      ? 'Ese partido ya empezo. No se toca.'
      : 'Ese partido ya cerro. No se toca.');
  }

  var antes = almacen.picksDe(usuario.id)[partido.id] || null;
  var pick = almacen.corregirPick(usuario.id, partido.id, eleccion);

  return {
    jugador: usuario.nombre,
    partido: partido.visitante.abbr + ' @ ' + partido.local.abbr,
    antes: antes ? antes.eleccion : null,
    ahora: pick.eleccion
  };
}
...
async function actualizarResultados(ctx) {
  ...
  for (var i = 0; i < semanas.length; i++) {
    ...
    (datos.events || []).forEach(function (ev) {
      ...
      // El que cuenta es el del minuto 60. Si hubo prorroga y ESPN no mando los
      // cuartos, no se guarda: mejor esperar que contar mal un empate.
      var normal = vivo.tiempoNormal(casa, fuera, estado, comp.status.period);
      if (!normal) return;
      var id = String(ev.id);
      ...
      res.partidos[id] = {
        ...
        tiempoNormal: { local: normal.local, visitante: normal.visitante },
        prorroga: normal.prorroga,
        cuenta: reglas.cuentaDeSemana(n),
