...
var CONFIG = {
  ...
  minutosCierre: 30,        // la semana cierra 30 min antes de su primer partido
...
/**
 * El momento exacto en que se cierra una semana completa: 30 minutos antes de
 * que arranque su primer partido (en la practica, el de jueves por la noche).
 * A partir de ese instante ya no se toca nada de esa semana.
 */
function cierreDeSemana(calendario, semana, config) {
  var juegos = partidosDeSemana(calendario, semana);
  if (!juegos.length) return null;
  var minutos = (config || CONFIG).minutosCierre;
  return new Date(Date.parse(juegos[0].inicio) - minutos * MINUTO);
}

/**
 * REGLA DE ORO: un partido que ya empezo no se puede elegir jamas.
 * Se evalua partido por partido, ademas del cierre de la semana completa, para
 * que ningun cambio de horario de la NFL abra una rendija.
 */
 ...
 *   'cerrado'  -> la semana ya cerro (faltan menos de 30 min para el primero)
 *   'iniciado' -> ese partido en concreto ya arranco o ya paso
 */
function estadoDePartido(calendario, partido, ahora, config) {
  if (yaInicio(partido, ahora)) return 'iniciado';
  var cierre = cierreDeSemana(calendario, partido.semana, config);
  if (cierre && ahora.getTime() >= cierre.getTime()) return 'cerrado';
  return 'abierto';
}
 ...
 *
 * El primero se lleva el premio gordo porque es el unico que hay que arriesgar
 * a ciegas: cuando arranca, la semana entera ya cerro.
 */
