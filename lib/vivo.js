...
// Con que marcador se cuenta cada semana (minuto 60 o final). Se pide tarde
// para no amarrar este modulo a las reglas al cargarse.
function reglasCuenta(semana) { return require('./reglas').cuentaDeSemana(semana); }
...
function algunoCorriendo(partidos, resultados, ahora) {
  return partidos.some(function (p) {
    ...
    if (ahora > arranco + DURA) {
      ...
      // su marcador final guardado (con el del minuto 60); puede haberse
      // retrasado, o haberse guardado antes de que existiera esa regla.
      var r = resultados.partidos[p.id];
      return !(r && r.final && r.tiempoNormal);
    }
    return true;
  });
}
...
/**
 * El marcador al minuto 60: la suma de los cuatro cuartos. Es el que cuenta
 * para la quiniela (ver reglas.marcadorQueCuenta).
 *
 * Devuelve { local, visitante, prorroga } o null si no se puede saber. Solo
 * no se puede cuando hubo prorroga y ESPN no mando los cuartos: ahi el final
 * no sirve, porque incluye el tiempo extra. Sin prorroga, el final ES el del
 * minuto 60.
 */
function tiempoNormal(casa, fuera, estado, periodo) {
  var detalle = (estado && estado.shortDetail) || '';
  var prorroga = Number(periodo) > 4 || /OT/.test(detalle);
  var lc = (casa.linescores || []).map(function (l) { return Number(l.value); });
  var lf = (fuera.linescores || []).map(function (l) { return Number(l.value); });

  if (lc.length >= 4 && lf.length >= 4 &&
      lc.slice(0, 4).every(Number.isFinite) && lf.slice(0, 4).every(Number.isFinite)) {
    var suma = function (a, b) { return a + b; };
    return {
      local: lc.slice(0, 4).reduce(suma, 0),
      visitante: lf.slice(0, 4).reduce(suma, 0),
      prorroga: prorroga || lc.length > 4 || lf.length > 4
    };
  }
  if (prorroga) return null;
  return { local: Number(casa.score), visitante: Number(fuera.score), prorroga: false };
}
...
function traducir(datos) {
  ...
  (datos.events || []).forEach(function (ev) {
    ...
    mapa[String(ev.id)] = {
      ...
      normal: est.completed ? tiempoNormal(casa, fuera, est, comp.status.period) : null
    };
  });
  return mapa;
}
...
async function deLaSemana(temporada, semana, partidos, resultados) {
  ...
  partidos.forEach(function (p) {
    ...
    // Sin el marcador del minuto 60 no se cierra: con prorroga, el final
    // podria dar un ganador donde para la quiniela hubo empate.
    if (!v.normal) return;
    var r = resultados.partidos[p.id];
    if (r && r.final && r.tiempoNormal) return;
    porCerrar.push({
      id: p.id,
      marcadorLocal: v.local,
      marcadorVisitante: v.visitante,
      tiempoNormal: { local: v.normal.local, visitante: v.normal.visitante },
      prorroga: v.normal.prorroga,
      cuenta: reglasCuenta(p.semana),
...
function semanasEnJuego(calendario, resultados) {
  ...
  calendario.partidos.forEach(function (p) {
    ...
    if (ahora > arranco + DURA) {
      var r = resultados.partidos[p.id];
      if (r && r.final && r.tiempoNormal) return;   // ya paso y ya esta contado
    }
